// types
import type { PokemonType, DataEvolutionType, DataPokemonType, DataSpeciesType } from '../types/pokeapi.types'

// ---------------------------------------------

const getIdFromUrl = (url: string): number => Number(url.split('/').filter(Boolean).pop())

export const pokemonAdapter = (dataPokemon: DataPokemonType, dataEvolutions: DataEvolutionType, dataSpecies: DataSpeciesType): PokemonType => {
